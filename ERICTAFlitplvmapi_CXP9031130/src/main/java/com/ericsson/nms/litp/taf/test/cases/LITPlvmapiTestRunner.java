...
public class LITPlvmapiTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPlvmapiTestRunner.class);
    ...
    @TestId(id = "CXP9031130-1", title = "Upgrade ERIClitplvmapi rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitplvmapiRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("lvmapi");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031130-2", title = "Run python test cases for ERIClitplvmapi package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitplvmapiTests() {
     ...
     * @PRE Execution of test {@link #runERIClitplvmapiTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031130-3", title = "Parse xml outputs from python tests")
